...
> **Status:** Recommendation document only. This document describes what *should be built next* — it does not implement anything, and this refresh does not change any code. A handful of items originally recommended here (see the "already landed" notes inside #1 and #6) have since shipped as real patches; they're marked done rather than deleted, so the roadmap stays an honest record instead of pretending nothing has moved.
> **Scope:** Nine structural improvements that would move Blackbox BOM from "impressive demo with a real backend hiding behind it" to "trustworthy product where every pixel reflects the database" (#1–#8), plus the genuinely-missing PLM capabilities that no amount of wiring or hardening can supply (#9).
> **Grounding:** Improvements #1–#8 are grounded in the original read-only subsystem audits (backend core, API surface, database schema, frontend architecture, frontend UI classification, and packaging/CI/testing), re-checked line-by-line against the current codebase for this refresh — current Alembic head is `050_rfq_headers_created_by_nullable`. Improvement #9 is grounded in `frontend/OPENBOM_GAP_ANALYSIS.md`'s competitive scoreboards. File paths and line numbers are quoted directly from source where useful. Where a feature is a **mock** or a **stub**, this document says so plainly, and where a target endpoint does not exist in the backend, this document says that plainly too — nothing here invents functionality that was not actually found in the code.
> **Related documents:** `frontend/OPEN_ITEMS.md`, `frontend/MIGRATION_MAP.md`, `frontend/OPENBOM_GAP_ANALYSIS.md`, `desktop/DESKTOP_PACKAGING.md`, `desktop/DURABILITY.md`, `DISASTER_RECOVERY_RUNBOOK.md`, `INSTALL.md`, `backend/TEST_FAILURES_TRIAGE.md`, `docs/audit-2026-08/FINDINGS_FULL_SCAN.md`, `docs/audit-2026-08/FIX_COVERAGE.md`, and the sibling docs `PROJECT_FEATURES_DOCUMENTATION.md`, `UI_UX_DOCUMENTATION.md`, `PROJECT_ARCHITECTURE.md`, `PATCHES_APPLIED.md`, `DEPLOYMENT_GUIDE.md`, `FIRST_TIME_SETUP.md`, `DATA_HANDLING.md`.
...
12. [Improvement #9 — Close the Genuinely-Missing PLM Feature Gaps](#improvement-9--close-the-genuinely-missing-plm-feature-gaps)
13. [Appendix A — Critical Bug Fixes That Should Ride Along](#appendix-a--critical-bug-fixes-that-should-ride-along)
14. [Appendix B — Cross-Reference Map](#appendix-b--cross-reference-map)
...
| 9 | Genuinely-missing PLM features (effectivity, multi-UOM, real import, non-SolidWorks CAD, requirements mgmt) | **High** (mixed; see sub-items) | XL (sum of parts) | Independent of #1–#8; competes for the same engineering time |
...
graph TD
    ...
    I9["#9 Genuinely-missing PLM features<br/>(effectivity, multi-UOM, real import,<br/>non-SolidWorks CAD, requirements mgmt)"]
...
4. **Drain the ECO notification queue.** `eco_service.py` correctly writes rows to `notifications_queue` on every notify-worthy ECO action (create/approve/reject/implement), and — as of this audit's re-check — correctly writes `EcoApproval` rows on approval too (this was previously an open gap; it is fixed: see `eco_service.py`'s `perform_eco_action`, which now creates an `EcoApproval` row with a real `approval_order` on the `"approve"` branch). What is still missing is the drain side: `email_service.py`'s `process_notification_queue(db, batch_size=50)` exists and works, but nothing calls it — no scheduler, no cron, no startup task. Approvers therefore still never receive a notification, not because the data isn't there, but because nothing ever reads it. Wiring the ECR/ECO UI to real `eco_approvals` data (as the wiring table below already recommends) will surface correct approval *state*; it will not fix delivery — that needs this queue actually drained (see Improvement #9e's scheduler dependency for the general shape of this problem, or add a minimal APScheduler/cron job calling `process_notification_queue` on an interval as a standalone fix, since it does not require any new backend logic — only an invocation).
5. **Close the routing-reads tenant-scoping gap.** `routing_api.py`'s process-plan list/detail reads are still unscoped raw SQL (see Improvement #6, item 6/1b) — fix this alongside any other work that touches that file, since #1's Phase 2 will start exercising it for real once a routing/manufacturing screen is wired.
...
The schema audit (single linear Alembic chain, current head `050_rfq_headers_created_by_nullable`, ~70 model modules) found the overall structure broadly coherent — tenant-scoped unique keys, a closure table for BOM explosion (`bom_closures`, migration 039), partial unique indexes done correctly in the Zoho tables — but surfaced a set of concrete **data-loss and integrity landmines**. Since the original audit, two of the items below have already been fixed and are recorded as such rather than removed, so the roadmap keeps an honest record of what shipped:

- **Already landed:** migration `048_index_foreign_keys` added the missing indexes described in item 8 below (all 30 previously-unindexed FK columns, including `bom_closures.ancestor_item_id`/`descendant_item_id`); migration `049_restore_check_constraints` closed the app-vs-DB constraint drift described in item 2 below (85 model-declared `CHECK` constraints restored, added `NOT VALID` so it does not abort on pre-existing non-conforming rows — see that migration's own docstring for the follow-up `VALIDATE CONSTRAINT` step). The dead tenant-isolation filter in item 6 is also already fixed: `app/core/tenant_events.py` now reads `execute_state.bind_mapper` (with a `# SECURITY` comment recording exactly why), not the nonexistent `mapper_` attribute — this closes Appendix A6.
- **Still open:** items 1 (over-aggressive `ON DELETE CASCADE` on `parts.primary_vendor_id` — confirmed still `ondelete="CASCADE"` in `app/models/part.py`, not yet `SET NULL` — plus the `boms`/`bom_templates`/`inventory_transactions` owner FKs), 3 (the inventory unique-constraint NULL hole), 4 (the two narrow `Numeric(10,4)` money columns), 5 (the inverted BOM adjacency-list attribute names), and 7 (`part_certifications` tenancy) remain exactly as originally found.
...
6. **FIXED — was the single most severe finding in the audits, a dead tenant-isolation filter.** The automatic ORM SELECT tenant filter in `app/core/tenant_events.py` used to read `execute_state.mapper_`, an attribute that **does not exist** on SQLAlchemy 2.0.48's `ORMExecuteState` (only `bind_mapper`/`all_mappers` exist); the resulting `AttributeError` was caught and swallowed, and a runtime reproduction confirmed the query returned **both tenants' rows** with the listener installed. **This is now fixed**: the code reads `execute_state.bind_mapper`, with an inline `# SECURITY` comment recording the history so nobody "simplifies" it back. What remains genuinely open, per a fresh read of the raw-SQL surface: `routing_api.py`'s two read endpoints (`GET /manufacturing/process-plans` list and `GET /manufacturing/process-plans/{id}`) still issue unscoped `SELECT * FROM process_plans` / `process_plan_steps` with no tenant filter at all — the module's own writes are correctly tenant-scoped (see its `tenant-security`/`tenant-insert-valuation` comments), but the two reads were not part of that fix and still leak cross-tenant process-plan data to any authenticated user who knows or guesses an id. This is tracked as a standing follow-up in `docs/audit-2026-08/FIX_COVERAGE.md` ("routing_api list/get_process_plan reads are still tenant-unscoped — inserts were fixed").
...
1. **DONE:** `tenant_events.py`'s `mapper_` → `bind_mapper` fix has shipped. What is still missing is the regression test that would have caught the original bug and would catch a future reintroduction — add a genuine cross-tenant read test (create two tenants, assert a query under tenant A's context never returns tenant B's rows) so the listener's correctness no longer depends on every service also filtering explicitly by convention.
1b. **Close the routing read-scoping gap (S):** add `tenant_sql_clause()` filters to `routing_api.py`'s `list_process_plans` and `get_process_plan` (and the analogous `process_plan_steps` join), matching the pattern already used by that file's write endpoints. This is a same-file, same-pattern fix — the hard part (establishing the raw-SQL tenant-scoping convention) is already done elsewhere in the module.
2. **Cascade-correction migration (next head after 050, i.e. `051_...`):** change `parts.primary_vendor_id` to `SET NULL`; change `boms.created_by`, `bom_templates.createdById`, and `inventory_transactions.performed_by` to `SET NULL` (or `RESTRICT` wherever an owner is meant to be mandatory). Write the rule down for future models: **CASCADE only within a true composition relationship** (a BOM cascading to its own items, a tenant cascading to its own rows) — **never on a reference or audit foreign key.** Audit the remaining ~70 models against that one rule while the migration is being written.
3. **Constraint-parity migration — DONE for the general case, verify the specific one.** Migration `049_restore_check_constraints` already restored all 85 model-declared `CHECK` constraints that migrated (non-`create_all`) databases were missing, added `NOT VALID` (so it does not abort on pre-existing non-conforming rows — the migration's docstring documents one known pre-existing violation, `projects.status='Fabricated'`, and instructs running `scripts/audit_check_constraints.py` then `ALTER TABLE ... VALIDATE CONSTRAINT` to close the gap on historical rows). Confirm the specific `InventoryTransaction`/`InventoryReservation` mismatch called out in the original audit is covered by that restoration, and if the Python `ALLOWED_REFERENCE_TYPES` set and the DB constraint's set still disagree on the `'receipt'`/`'issue'`/`'sales_order'` values, add a small test that reads both from one shared source of truth so they cannot drift apart again.
...
8. **Index review pass — the FK-column sweep is DONE, keep going for query-shape indexes.** Migration `048_index_foreign_keys` already indexed all 30 previously-unindexed FK columns found in the original audit, including `bom_closures.ancestor_item_id`/`descendant_item_id`. What that migration does not cover is composite/query-shape indexes for the access patterns #1's newly-wired screens will start exercising for real (audit-logs filtered by `entityType` + date range, inventory filtered by part + warehouse, quality lists filtered by status) — measure with `EXPLAIN` on realistic data volumes rather than guessing which indexes matter, and add composite indexes where the FK-column sweep alone isn't enough.
...
- **The checks that do run mostly don't gate anything:** mypy is invoked with `|| true`, the frontend ESLint step is `2>/dev/null || true`, and `tsc --noEmit` is likewise `|| true`; `postgres-ci.yml`'s SQLite test suite runs `continue-on-error` with roughly 93 documented pre-existing failures tracked in `backend/TEST_FAILURES_TRIAGE.md`. The only hard gates today are `ci.yml`'s questionable `test-backend` job, the vitest job, and `postgres-ci.yml`'s fresh-install job — which asserts a hardcoded `EXPECTED_HEAD` against `alembic heads`. That value has correctly been kept up to date (`050_rfq_headers_created_by_nullable` today, matching the real chain), so it is not currently stale — but it is still a hardcoded string that someone must remember to bump on every new migration, exactly the kind of manual step that silently drifts if the bump is ever missed. Deriving it dynamically (this improvement's step 1) removes that recurring manual chore rather than fixing a live bug.
...
## Improvement #9 — Close the Genuinely-Missing PLM Feature Gaps

> **Everything in Improvements #1–#8 is about wiring or hardening what already exists.** This item is different: these five sub-items are real gaps in the product's *data model and backend*, not UI plumbing over a finished backend. `frontend/OPENBOM_GAP_ANALYSIS.md` is the source of record for all five (its per-domain scoreboards, and its "Genuinely new work" list under Phase 8c). Fixing #1–#8 alone would leave a fully-wired, fast, well-tested product that still cannot do these five things, because the backend tables and routes for them do not exist yet.

Beginner note: "PLM" (Product Lifecycle Management) is the category of software this product competes in — it manages a part/BOM from design through manufacturing, change, and obsolescence. The five gaps below are the specific PLM capabilities that OpenBOM (and most PLM/PDM competitors) have natively and this product does not yet have at all, as opposed to having-but-not-wired-to-a-screen.

### 9a — Effectivity (date / serial / lot) on BOM Lines

**Problem.** `frontend/OPENBOM_GAP_ANALYSIS.md` calls this "the single largest true gap" in the Change Management domain. A grep for `effectivity` across the entire backend returns nothing except `eco_headers.effective_date` — one date column on the ECO header itself. There is no `effective_from`/`effective_to` on `bom_items`/`bom_items_master`, and no serial- or lot-break effectivity model anywhere.

**Why it matters.** Effectivity is how a real manufacturing BOM answers "which revision of this part do I build with, starting when, or starting at which serial/lot number?" Without it, an ECO's "effective date" is descriptive text with no enforcement: nothing stops (or even flags) building unit #4,512 with the pre-change part when the change was supposed to take effect at unit #4,500. This is a compliance and traceability gap, not a cosmetic one, in a tool whose regulated-industry roadmap already leans on e-signatures and audit trails elsewhere.

**Current limitation.** A BOM line is either in the BOM or not; there is no way to express "part X through end of lot 12, part Y from lot 13 onward" or "part X until 2026-09-01, part Y after." The closure-table BOM explosion (`bom_closures`) has no time or serial dimension to filter on.

**Recommended solution.** Add `effective_from_date`/`effective_to_date` (nullable) and `effective_from_serial`/`effective_to_serial` (nullable string, since serials aren't always numeric) to `bom_items`; add a migration; extend `bom_service`'s explosion/rollup queries to accept an as-of date or serial and filter lines whose effectivity window excludes it (default: no filter, so existing behavior is unchanged when the feature isn't used); surface the fields on the BOM editor line-item form and add an "as-of" selector to the BOM viewer. Land the model + migration + explosion-query change first (pure backend, testable in isolation) before touching the UI.

**Benefits.** Closes the largest single gap flagged in the competitive analysis; unlocks a real "what does unit #N actually contain" query, which downstream quality/traceability screens can build on.

**Risks.** Explosion-query changes touch a performance-sensitive path (`bom_closures` joins) — benchmark before and after; a badly-designed effectivity UI can make simple BOMs feel more complex, so default to no-effectivity-set behaving exactly as today.

**Effort:** L. **Priority: Critical** for the regulated/manufacturing segment of the roadmap; **High** generally. **Dependencies:** none — can start immediately; benefits from #6's cascade/constraint work landing first since it touches the same tables.

### 9b — Multi-UOM with Conversion

**Problem.** `parts.uom` is a single free-text string (e.g. `"ea"`, `"m"`, `"kg"`) with no conversion table or route at any layer. `frontend/OPENBOM_GAP_ANALYSIS.md` calls this "the one true zero" in the Parts domain.

**Why it matters.** A BOM that mixes a part bought in meters with a part consumed in feet, or a part stocked in kilograms but ordered in pounds, cannot roll up cost or quantity correctly today — the rollup simply sums whatever string is in `uom` with no conversion, silently producing a wrong total whenever units differ across a BOM.

**Current limitation.** No conversion factors exist anywhere; the closest working analog in the codebase is the currency `exchange_rates` table/service, which already solves the identical shape of problem (value in unit A → value in unit B via a stored factor) for money instead of quantity.

**Recommended solution.** Copy the `exchange_rates` pattern: a `uom_conversions` table (`from_uom`, `to_uom`, `factor`, optionally scoped by unit *dimension* — length, mass, volume, count — so `m→ft` is never offered as a candidate conversion for a mass unit), a small conversion service function analogous to the exchange-rate lookup, and a normalized `uom` reference list (replacing free text with a constrained set, migrating existing values). Wire BOM rollup and cost-extension calculations to convert through a single canonical unit per dimension before summing. Seed the table with the standard SI/imperial pairs for length, mass, and volume; let tenants add custom conversions for anything domain-specific (e.g. reels, spools).

**Benefits.** Fixes silently-wrong rollups on any BOM that mixes units — a correctness fix, not just a feature; reuses an already-proven pattern (`exchange_rates`) instead of inventing a new one.

**Risks.** Migrating free-text `uom` values to a normalized reference list will surface inconsistent historical data (typos, synonyms like `"pcs"` vs `"ea"`) — plan a data-cleanup pass, not just a schema change.

**Effort:** M. **Priority: Medium.** **Dependencies:** none.

### 9c — Real CSV/XLSX Bulk Import

**Problem.** Two half-implemented import paths exist and neither actually creates data: the client-side CSV import fills an unsaved grid with no API call unless the BOM is separately saved by the user; the server-side pipeline (`bulk_import.py`) stages rows into `bulk_import_rows` and marks them `"processed"` **without ever creating a `Part`**. No XLSX reader exists at all — only CSV.

**Why it matters.** "Drag your spreadsheet in" is the standard onboarding motion for this entire product category (`frontend/OPENBOM_GAP_ANALYSIS.md` calls out OpenBOM's whole onboarding flow as exactly this). A new tenant's first action is almost always importing an existing parts list or BOM from a spreadsheet — and today that action silently does nothing on the backend path and requires exact manual grid entry on the frontend path.

**Current limitation.** `bulk_import_jobs`/`bulk_import_rows` tables and their status-tracking CHECK constraints exist (restored correctly by migration 049); the plumbing to actually parse a row into a `Part` (or a `BomItem`) does not.

**Recommended solution.** Phase 1 (S–M): finish the CSV path — have `bulk_import.py`'s row processor actually call `part_service.create_part` (or update, on a matched part number) per staged row, with per-row error capture back into `bulk_import_rows.status`/`error_message` so partial-failure imports are visible, not silent. Phase 2 (S): add an XLSX reader (`openpyxl`, already a common, lightweight dependency — check `backend/requirements.txt` before adding a new one) alongside the existing CSV path, sharing the same row-validation/creation logic. Phase 3 (S): make the frontend's client-side import call the real endpoint immediately per row (or per batch) instead of staging unsaved grid rows, matching the "no fabricated fallback, no silent no-op" standard the rest of Improvement #1 is enforcing elsewhere.

**Benefits.** Turns the single most common new-tenant onboarding action from "does nothing" into "actually works" — plausibly the highest ratio of user-visible impact to engineering effort in this whole document, since the staging tables and status model already exist.

**Risks.** Bulk creation needs the same tenant-scoping and duplicate-detection discipline as the rest of the parts API (reuse `part_service`'s existing duplicate-check endpoint rather than re-implementing it inline); large files need a size/row-count cap and a background-job path rather than blocking the request.

**Effort:** M. **Priority: High** — was flagged Critical in the original gap analysis; downgraded to High only because the frontend now at least shows an honest (if unsaved) grid rather than fabricating success. **Dependencies:** none; can reuse #1's "no fabricated fallback" wiring standard as the acceptance bar for the frontend phase.

### 9d — Non-SolidWorks CAD Connectors (Fusion 360, Onshape, Altium)

**Problem.** The SolidWorks integration is genuinely deep — a real ~150KB .NET add-in, 24 backend routes, dedicated plugin auth, and a CAD Vault UI at `/pdm` (see `PROJECT_ARCHITECTURE.md`/`PROJECT_FEATURES_DOCUMENTATION.md` for the SolidWorks specifics). Every other CAD system named in the competitive analysis — Fusion 360, Onshape, Altium/ECAD — has **zero lines of code** anywhere in the backend or frontend.

**Why it matters.** A design tool's CAD integration is only as good as the CAD tools its customers actually use. Every competitor in the gap analysis ships at least one non-SolidWorks connector; a SolidWorks-only shop is a real but narrowing slice of the mechanical-design market, and Altium/ECAD coverage matters specifically for electronics BOMs (component placement, refdes, netlist import) that this product's manufacturing focus otherwise targets well.

**Current limitation.** The generic web-facing `/cad/apply-sync` path exists but writes nothing to the database — it is a stub that returns a misleading "Sync complete · N changes applied · audit logged" toast (already flagged as a bug in Appendix A-adjacent findings; file alongside this work if not already fixed). There is no vendor-neutral CAD import contract to build additional connectors against — the SolidWorks integration was built CAD-specific from the start, not as an instance of a generalized interface.

**Recommended solution.** Do not build all three at once. Pick the highest-value target first (Altium for electronics-BOM alignment with this product's manufacturing/procurement focus, or Fusion 360 for the broadest mechanical-CAD market share — a product decision, not an engineering one) and build one real connector end to end: a minimal read path (import a BOM/parts list export from the tool's native or neutral exchange format — e.g. Altium's `.BomDoc`/CSV export, or Fusion 360's BOM export API) before attempting live bidirectional sync like the SolidWorks add-in has. Extract the parts of the SolidWorks integration that are genuinely CAD-agnostic (document versioning, the vault stats/tree queries, the derivative-file model) into a shared CAD-connector interface so the second and third connectors are markedly cheaper than the first. Fix the misleading `/cad/apply-sync` toast regardless of which connector ships first — a fabricated "audit logged" success message is a trust problem on its own.

**Benefits.** Materially widens the addressable market beyond SolidWorks shops; the shared-interface extraction pays for itself starting with the second connector.

**Risks.** Each CAD vendor's export/API surface is genuinely different — resist the temptation to over-generalize the shared interface before a second real connector proves which parts of the SolidWorks-derived abstraction actually generalize versus which were SolidWorks-specific all along.

**Effort:** XL for full parity across three connectors; L for a single, well-chosen first connector with a read-only import path. **Priority: High** (P1 in the gap analysis's prioritized list) but sequenced behind #1–#3 for engineering bandwidth, since it is new capability rather than unlocking already-built value. **Dependencies:** none technical; a product decision on which CAD system to target first is the actual blocker.

### 9e — Requirements Management

**Problem.** `frontend/OPENBOM_GAP_ANALYSIS.md` is explicit: "no table, route, or UI at all; the one item in this domain with nothing behind it." There is no requirements model anywhere in the ~70 model modules audited.

**Why it matters.** Requirements management — tracking a customer/regulatory/engineering requirement and linking it to the parts/BOM lines that satisfy it — is a standard differentiator competitors (Arena, Teamcenter-class tools, and OpenBOM itself per the gap analysis) offer natively. Its absence is most acutely felt anywhere the product touches compliance or regulated-industry work, since "which requirement does this part satisfy, and can I prove it" is exactly the kind of traceability question an auditor asks.

**Current limitation.** Nothing — this is a from-scratch build, not a wire-up. There is no partial implementation to extend.

**Recommended solution.** Start minimal: a `requirements` table (id, tenant-scoped, title, description, source — customer/regulatory/internal, status, owner) plus a many-to-many `requirement_part_links` (or `requirement_bom_item_links`) join table, basic CRUD routes following the same `response_model`/service-layer pattern the newer, well-structured endpoints already use (see Improvement #3's schema-consolidation guidance — build this one the *right* way from day one rather than adding to the inline-Pydantic sprawl), and a simple list/detail screen plus a "linked requirements" panel on the part/BOM-item detail view. Do not attempt requirement-to-test-case traceability, formal verification workflows, or import from dedicated requirements tools (DOORS, Jama) in the first pass — that is a distinct, much larger effort and should be scoped separately if there's real customer demand for it.

**Benefits.** Closes the one true zero in the data model; unlocks basic requirements-to-part traceability, which is disproportionately valuable for the regulated-industry / compliance-conscious segment of the roadmap even at minimal scope.

**Risks.** Scope creep is the main risk — "requirements management" can expand indefinitely (versioning, baselines, formal verification matrices); hold the first release to CRUD + linking + a list/detail screen and treat anything beyond that as a follow-up improvement of its own.

**Effort:** M for the minimal scope described above; XL if scope grows to include verification/traceability-matrix workflows. **Priority: High** (per the gap analysis's P2 ranking) but explicitly a "build small first" item. **Dependencies:** none; benefits from #3's schema-consolidation pattern being available so this new domain starts clean instead of adding another inline-Pydantic file.

### Improvement #9 — Effort / Priority / Dependencies (rollup)

| Sub-item | Effort | Priority | Dependencies |
|---|---|---|---|
| 9a Effectivity on BOM lines | L | Critical (regulated segment) / High (general) | None; touches tables #6 also touches |
| 9b Multi-UOM conversion | M | Medium | None |
| 9c Real CSV/XLSX import | M | High | None; reuses #1's honesty standard |
| 9d Non-SolidWorks CAD connectors | L (one connector) – XL (all three) | High | Product decision on target CAD system |
| 9e Requirements management | M (minimal) – XL (full traceability) | High | Benefits from #3's schema pattern |
...
| A6 | **FIXED.** The tenant SELECT auto-filter was dead code (`execute_state.mapper_` doesn't exist in SQLAlchemy 2.0.48); now reads `execute_state.bind_mapper`, confirmed in `backend/app/core/tenant_events.py`, with an inline `# SECURITY` comment against regression. Still needed: the cross-tenant regression test that would have caught it originally (see #6 step 1), and closing `routing_api.py`'s two still-unscoped read endpoints (#6 step 1b) | `backend/app/core/tenant_events.py` | Kept here as a record of what shipped, not as an open item. Part of #6 |
...
| `docs/audit-2026-08/FINDINGS_FULL_SCAN.md` | The 74-finding full-repo line-by-line scan this document's corrections were re-checked against | #1, #6, #9 |
| `docs/audit-2026-08/FIX_COVERAGE.md` | The fixed/deferred ledger for those 74 findings (43 fixed / 31 deferred); source for the "already landed" notes throughout #1 and #6, and for the standing follow-ups (routing read-scoping, notification-queue scheduler) | #1, #6 |
| `frontend/OPENBOM_GAP_ANALYSIS.md` | Competitive gap analysis vs. OpenBOM/Arena/Teamcenter-class tools (65% weighted / 49% reachable); source for every sub-item in #9 | #9 |

---

### Final word

The order matters less than the *first* step: **start #1 immediately, land Appendix A's Phase-0 fixes in the same week, and let #6's remaining cascade migration and routing-read-scoping fix ship ahead of everything else** (its FK-index and check-constraint items, and the tenant-filter fix in Appendix A6, have already landed since the original audit — recorded above, not repeated as open work). The audits repeatedly note how much genuinely already works — 549 real backend routes, hardened auth, a coherent multi-tenant schema, a thoughtful local-first desktop story, and (as of this refresh) 43 of 74 audit findings already fixed. The product's biggest problem is still that the UI tells demo-era stories instead of showing the real system sitting right underneath it — wire it up (#1), make the wiring type-safe (#3), and put a test harness around it (#7) so it never drifts apart again. Separately, and on its own timeline, #9 is where real net-new PLM capability lives: effectivity, multi-UOM, working bulk import, more CAD connectors, and requirements management are gaps in the product, not gaps in the wiring, and no amount of #1–#8 closes them.
