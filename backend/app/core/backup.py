...
async def restore_physical_backup(
    ...
    try:
        ...
        # Platform-aware restore_command: Postgres runs it via the host shell,
        # which is cmd.exe on the Windows desktop bundle (no `cp`) and sh on Linux.
        if os.name == "nt":
            _restore_cmd = f'copy /Y "{os.path.join(settings.WAL_ARCHIVE_DIR, "%f")}" "%p"'
        else:
            _restore_cmd = f"cp {settings.WAL_ARCHIVE_DIR}/%f %p"
        recovery_conf.write_text(
            "# Blackbox BOM PITR recovery configuration\n"
            "# Generated by restore_physical_backup\n"
            f"restore_command = '{_restore_cmd}'\n"
