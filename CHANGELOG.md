...
## [Unreleased]
**Green Postgres hard gate + migration/test hardening; Members UI, real CAD viewer, document downloads, FK indexing.**

### Added
- **Members & Privileges screen** — `frontend/src/components/screens/MembersScreen.jsx`, routed at `/members` (`src/screens/App.jsx`) with a nav entry in `NavRail.jsx`. Lists team members, assigns roles and enables/disables accounts through the existing RBAC API, which until now had no UI at all.
- **Real 3D CAD viewer** — `frontend/src/components/cad/CadViewer.jsx`. STEP/STP/IGES/IGS are tessellated in-browser by `occt-import-js` (a WASM OpenCascade build, `^0.0.23`); STL/OBJ/GLTF/GLB/PLY/3MF load through `three` (`^0.184.0`) loaders. Proprietary native CAD (`.sldprt`, `.sldasm`, `.ipt`, `.iam`, `.prt`, `.catpart`) is explicitly unsupported — the viewer says so rather than failing silently.
- **`GET /api/v1/documents/{id}/download`** (`backend/app/api/endpoints/documents.py`) — the backend had **no file-download endpoint at all**: the vault could list files and accept uploads, but nothing could ever fetch the stored bytes back, so downloads, previews and the 3D viewer had no data source. Handles both S3-backed and local storage. Local files are resolved with `os.path.realpath` and rejected unless they stay inside the upload root, so a tampered or legacy `filePath` row cannot read arbitrary files off the server.
- **`backend/scripts/seed_e2e_fixture.py`** — seeds a multi-level BOM fixture so end-to-end tests exercise real data paths; `--clean` removes it again.
- **End-to-end Playwright suite** — `frontend/e2e/real-flows.spec.js` (API reachable through the app origin rather than the SPA fallback, anonymous callers rejected, wrong password rejected, real login reaches the app shell, authenticated screens render without crashing) alongside the existing `smoke.spec.js` and `auth.setup.js`. Run manually; there is no Playwright job in CI on this branch.

### Changed
- **Postgres CI is now a HARD GATE and passes green.** The full pytest suite runs against a real PostgreSQL 16 service (`Test Suite on Postgres` job). Session-scoped asyncio loops (required for asyncpg) are passed via `-o` on the CLI so the fast SQLite/dev track keeps function-scoped isolation. A red here blocks merge. The suite has since grown to **648 collected tests**; the green result recorded when this gate landed (634 passed / 0 failed / 1 skipped / 1 xfailed) has not been re-measured against the newer tests.
- Alembic migration head advanced to `048_index_foreign_keys` (48 migrations; linear chain …044_compliance_evals → 041_zoho_books → 045 → 046 → 047 → 048). Fresh install now builds **160 tables**. Registered the previously-unregistered SolidWorks contract router at `/api/v1/solidworks`.
- **Every foreign-key column is now indexed** (migration `048_index_foreign_keys`). 30 FK columns had no supporting index, which cost twice: joins across them fell back to sequential scans — including `bom_closures.ancestor_item_id`/`descendant_item_id`, which BOM explosion and where-used join on for every lookup — and 28 of the 30 are `ON DELETE CASCADE`/`SET NULL`, so deleting a parent scanned the whole child table. Index names follow SQLAlchemy's `ix_<table>_<column>` default so migrated and freshly-created schemas stay identical. No FK column is left without index coverage.
- **CAD import modal is honest about what it can do** — `CADImportModal.jsx` used to run a fake progress bar and then show a hardcoded parts list. It now moves to an `addin-required` step explaining that `.sldasm`/`.sldprt` are proprietary SolidWorks formats the browser cannot read, and that the SolidWorks add-in is required. Nothing is uploaded and no import runs.
- **`AppCtx` no longer falls back to the bundled demo BOM** — `rows` was `apiRows || data.rows`, so before the parts API responded (and forever, if it failed) Analytics, the BOM editor, search and sourcing all silently rendered the DEMO assembly: no error, no empty state, just plausible numbers that were not the user's. It now defaults to `[]`. Because `[]` is truthy it also wins the downstream `ctx?.rows || BOM_DATA.rows` expressions, so those fallbacks are neutralised too.
- **`window.*` app-global → ES module migration (improvement #2) continued.** `window.__nav` is replaced by `src/services/navigation.js`, `window.__poDraft` by `src/services/poDraft.js`, `window.INR_RATE` by `src/utils/currency.js`, and `apiConnected` now travels on the React context instead of the window object. One live `window.INR_RATE` read remains, in `src/utils/download.ts`. The migration is not finished.
