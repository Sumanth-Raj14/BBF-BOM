...
- **Alembic head:** `064_optimistic_lock_version` (single, linear · 64 migrations). Verified this pass via `alembic heads`.
- **Test status (measured this pass):** backend **991 passed / 3 failed / 1 skipped / 1 xfailed** on the SQLite fast track (32m). Two of the three failures are `test_search.py` full-text tests that CANNOT pass on SQLite (`ILIKE`, tsvector) — they are Postgres-only by nature, not regressions. The third was `test_migration_chain_single_linear_head` asserting the head's NAME; it now asserts the topology instead, so a new migration no longer fails it. Frontend: **316 passed / 316** across 61 files, green on 5 of 6 consecutive runs.
- **State:** feature-complete + published. Production-readiness gaps closed 2026-08-23: optimistic locking, shipped TLS config, bundle size, unbounded log reads (see §6, §10, §12).
...
- **Optimistic locking (migration 064):** `parts`, `bom_items` and `bom_templates` carry `lock_version`, wired to SQLAlchemy's `version_id_col` via `app/models/mixins.OptimisticLockMixin`. Every ORM UPDATE/DELETE on those tables carries `AND lock_version = <loaded>`; the loser of a race raises `StaleDataError`, which `app/main.py` maps to **409** (not 500 — a 500 invites a retry that clobbers the write it just lost to). Before this, concurrent edits to one BOM line silently discarded one of them. Named `lock_version` because `boms.version` already means a user-chosen business revision. Raw `text()` and bulk Core update/delete do NOT participate, exactly as they bypass the tenant auto-filter.
...
- **Frontend unit tests:** `cd frontend; npx vitest run` — **316 passed / 316** across 61 files (measured 2026-08-23). `vitest.config.ts` sets `testTimeout: 20000`: the 5s default failed a *different* test each run under load (57s of jsdom setup alone), which trains everyone to ignore red.
- **Query-count benchmark:** `python -m scripts.benchmark_queries` counts SQL round-trips and fails if a read scales with row count. `benchmark_bom.py` runs on in-memory SQLite where N+1 is invisible by construction. Current verdict on `list_bom_items`: 3 queries at 10 lines, 3 at 100 — scale-invariant.
- **End-to-end (Playwright):** `frontend/e2e/` holds `smoke.spec.js`, `real-flows.spec.js` (auth reachability, anonymous rejection, wrong-password rejection, real login, per-screen crash checks) and `auth.setup.js`. `backend/scripts/seed_e2e_fixture.py` seeds a multi-level BOM to run them against. **There IS now an `e2e-playwright` job in `.github/workflows/postgres-ci.yml`** (verified 2026-08-23; it seeds `bom_e2e_db` via `scripts/seed_e2e_fixture.py`); the CI wiring and the `write-flows.spec.js` suite live on the unmerged branch `test/e2e-ci-and-write-flows`.
...
- **HTTPS / production mode:** `ENVIRONMENT=production` makes the backend mark auth cookies `Secure`, and browsers never send a Secure cookie over `http://` — so production mode without TLS is a total outage that reads as an app bug (login 200, then 401 on everything). Either terminate TLS upstream, or use the shipped overlay:
  ```sh
  sh backend/scripts/generate-self-signed-cert.sh bom.yourcorp.local
  docker compose -f docker-compose.yml -f docker-compose.tls.yml up -d
  ```
  `frontend/nginx.tls.conf` (:443, TLS 1.2 floor, HSTS, ACME path kept open for renewals) and `frontend/nginx.conf` (:80) both `include /etc/nginx/snippets/app-locations.conf`, so a route fixed for HTTP cannot silently stay broken over HTTPS. The generated cert is for internal hostnames no public CA will issue for; use a real CA cert for anything internet-facing.
...
- **`backend/test.db` is a SHARED FILE**, not `:memory:` (conftest falls back to `sqlite+aiosqlite:///./test.db`). Two concurrent pytest processes corrupt each other: the second fails at *fixture* setup with `UNIQUE constraint failed: tenants.id` and cascading `PendingRollbackError`, which looks exactly like a broken new test. It also slows the full run enormously via file-lock contention (observed >50 min for a ~32 min suite, never finishing). Give a concurrent run its own DB via `TEST_DATABASE_URL=sqlite+aiosqlite:///./scratch.db`.
...
- **GitHub Actions secrets + environments are EMPTY** — zero secrets, zero environments. Both deploy jobs in `ci.yml` are therefore deliberately unreachable (`deploy-staging` gated on a `main` branch that does not exist). They need `STAGING_/PRODUCTION_HOST|USER|SSH_KEY` plus the `staging`/`production` environments. **User-only.** The image-pull side is now fixed: compose services carry an overridable `image:` key and the deploy jobs export `BACKEND_IMAGE`, so `docker compose pull` fetches what CI built instead of resolving nothing.
- **Desktop packaging is Windows-only** — `.ps1` build scripts and `fetch_postgres.ps1`, no Linux/macOS target. The *server* path (docker compose) is cross-platform, so this only blocks a per-workstation desktop install on non-Windows. Not started; it needs per-OS PyInstaller specs, a bundled Postgres per platform, and signing.
